...
- Fix: устранён кейс "залипания" негативного `Slow`-статуса после краша/жёсткого выхода клиента; stale `walkspeed:intoxicated` теперь подавляется, если фактическая интоксикация равна `0`.
